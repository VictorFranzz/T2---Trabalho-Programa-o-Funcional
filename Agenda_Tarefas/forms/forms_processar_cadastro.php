...
switch($_REQUEST['req']) {   
    ...
    case 'inserirCadastro':
        ...
        if($bValidaQuery) {
           ...
           $oRetorno->mensagem = 'Houve uma falha ao incluir no registro.';
        }   
    ...
    case 'excluirTarefa':
        ...
        if($bValidaQuery) {
           ...
           $oRetorno->mensagem = 'Houve uma falha ao excluir no registro.';
        }   
    ...
    case 'alterarTarefa':
        $oDados        = getObjetoDados();
        $oConexao      = new ConexaoBanco('localhost', 'root', '', 'agendatarefas');
        $sDelete       = getUpdate($oDados);
        $bValidaQuery  = mysqli_query($oConexao->oConexao, $sDelete);
        $oRetorno      = new stdClass();
    
        if($bValidaQuery) {
            $oRetorno->mensagem = 'Registro Alterado com sucesso.';
        }   
        else {
           $oRetorno->mensagem = 'Houve uma falha ao alterar no registro.';
        }   
    ...
    case 'concluirTarefa':
        ...
        if($bValidaQuery) {
           ...
           $oRetorno->mensagem = 'Houve uma falha ao concluir no registro.';
        }   
